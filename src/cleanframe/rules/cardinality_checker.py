...
class CardinalityChecker(BaseRule):
    def detect(self, df: Any, params: dict[str, Any]) -> list[Decision]:
        ...
        for col in ndf.columns:
            ...
            # Case 2: High-cardinality string/object columns
                ...
                str(dtype).lower() in {"str", "string", "object", "utf8"}
                and row_count > 0
                ...
                decisions.append(
                    Decision(
                        ...
                        action="flag_id",
                        parameters={
                            "unique_ratio": n_unique / row_count,
                            "reason": "High-cardinality candidate ID",
                        },
                        signal_strength=0.9,
                        rationale=(
    ...
    def transform(self, df: Any, decisions: list[Decision]) -> Any:
        columns_to_drop = [d.column for d in decisions if d.action == "drop_column"]
    ...
    def explain(self, decisions: list[Decision]) -> str:
        ...
        if dropped:
            lines.append(
                "The following columns were dropped for having only a single unique value:"
            )
            for col in dropped:
                lines.append(f"  • '{col}' (constant)")
        if flagged:
            lines.append(
                "The following columns were flagged as likely identifier columns due to their high cardinality (>99% unique):"
            )
            for col in flagged:
